...
result is a linear system, wich is solved for $x{n+1}$.
\subsubsection{Solving the linear system}
Solution of dense linear systems is of order $\mathcal O (n^3)$. This,
as well as the memory cost of keeping $n^2$ entries, is prohibitively
large for our problems, where $n$ can be around
$100\,000$. Thankfully, our matrices are sparse (because component
number $i$ of $f$ only depends of the cells connected to cell number
$i$), which yields huge savings in memory and computation time : we
only store non-zero entries in the matrix, and use specialized
...
ill-conditioned problems, this iterative algorithm may not converge fast enough : we
then fall back to a direct method, which is slower and less precise
but has no convergence issues.
