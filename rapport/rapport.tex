...
la stabilisation : expliquer comment. M\'ethode de clusters.
...
sur des courbes
\subsection{Influrence of stabilisation}
...
easy task. We want to remove spurrious oscillations caused by small
$\lambda$, as well as avoid intra-clusters diffusion. Figure
\ref{fig:lambdas} shows the pressure at mid-width for our problem with
different $\lambda$. For small values of $\lambda$ (under $10^{-5}$),
we can see the even-odd oscillations. As $\lambda$ increases, clusters
(here, of two cells) appear, until a stage around $10^{-4}$ where
pressure becomes constant inside the clusters.
...
TODO interfacer triangle avec le programme, ou mesher en
rectangles. Va poser des probl\`emes en stationnaire parce que
turbulent, essayer en instationnaire et voir si il y a solution
stationnaire dans la chemin\'ee
\section{Conclusion}
TODO
\listoffigures
\listoftables
